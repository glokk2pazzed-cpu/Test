// ==UserScript==
// @name         DiscordCorruptControl ELITE â€” 2.1.0 (Enhanced)
// @namespace    https://github.com/glokk2pazzed-cpu
// @version      2.1.0
// @author       @ogunworthy (enhanced)
// @description  CorruptControl loader + mobile keyboard scroll lock + watermark polish. Original v2.0.6 loader preserved verbatim.
...
/* ============================================================================
   CorruptControl ELITE 2.1.0  â€”  enhancement layer
   ----------------------------------------------------------------------------
   - Tampermonkey + Violentmonkey compatible (GM.* polyfills below).
   - Original 2.0.6 loader script is included VERBATIM after this section.
     Nothing has been removed. All original behavior (toggle button image,
     beta-script eval, watermark, command system, key system inside
     corrupt.beta.js) runs exactly as before.
   - This top section ADDS:
       * Mobile keyboard scroll lock for Discord web (chat stays pinned
         to the newest message when the soft keyboard opens â€” no more
         being yanked up to old "unread" messages on iOS/Android).
       * Watermark polish (cleaner glass card, readable text, draggable
         clamp inside viewport, mini-mode tap toggle, no-overlap with
         Discord composer on mobile).
       * Cross-engine GM polyfill so callbacks behave the same in VM/TM.
   ============================================================================ */

(function ELITE_ENHANCEMENT_LAYER() {
    'use strict';
    if (window.__CC_ELITE_ENH__) return;
    window.__CC_ELITE_ENH__ = true;

    /* ---------- GM polyfill (Violentmonkey async GM.* <-> GM_* sync) ----- */
    try {
        if (typeof GM_getValue === 'undefined' && typeof GM !== 'undefined' && GM.getValue) {
            // VM exposes async GM.*; some legacy code expects sync GM_*. Provide best-effort wrappers.
            window.GM_getValue = (k, d) => { try { return GM.getValue(k, d); } catch { return d; } };
            window.GM_setValue = (k, v) => { try { return GM.setValue(k, v); } catch {} };
            window.GM_deleteValue = (k) => { try { return GM.deleteValue(k); } catch {} };
            window.GM_xmlhttpRequest = (o) => { try { return GM.xmlHttpRequest(o); } catch {} };
        }
    } catch (_) {}
    ...
    /* ---------- Mobile keyboard scroll lock --------------------------------
       Problem: on Discord web mobile, when the composer is focused the
       visualViewport shrinks and the message list reflows; Discord then
       restores the wrong scroll anchor and the user is dumped at older
       messages, looking like "unread you didn't read".
       Fix: detect chat scroller, remember it's at-bottom before keyboard
       open, then re-pin to bottom after every viewport/resize/focus event
    ...
    (function mobileScrollLock() {
        ...
        const getScroller = () => {
            const list = document.querySelectorAll(SCROLLER_SEL);
            // Pick the visible one closest to viewport bottom
        ...
        const schedulePin = () => {
            if (!composerFocused || !wasAtBottom) return;
        ...
        // Track composer focus
        document.addEventListener('focusin', (e) => {
            const t = e.target;
            if (!t || !t.matches) return;
            if (t.matches(COMPOSER_SEL)) {
                const el = getScroller();
                wasAtBottom = nearBottom(el);
        ...
        document.addEventListener('focusout', (e) => {
            const t = e.target;
            if (!t || !t.matches) return;
            if (t.matches(COMPOSER_SEL)) composerFocused = false;
        }, true);

        // VisualViewport is the canonical signal for soft-keyboard show/hide.
        ...
        // Whenever the user types, re-pin (Discord inserts pending text rows that nudge scroll).
        document.addEventListener('input', (e) => {
            if (composerFocused) schedulePin();
        }, true);

        // CSS: make sure the composer area is always above the soft keyboard
        // and the message list reserves room for it. Uses Discord's mobile vars.
        const style = document.createElement('style');
        style.id = 'cc-mobile-scroll-fix';
        style.textContent = `
            @supports (height: 100dvh) {
                html, body, #app-mount { height: 100dvh !important; }
            }
            /* Keep composer pinned above keyboard */
            ...
            /* Hide jump-to-present banner flicker on focus */
            [class*="jumpToPresentBar"] { transition: opacity .15s ease; }
        `;
        (document.head || document.documentElement).appendChild(style);
    })();

    /* ---------- Watermark polish ------------------------------------------
       Restyles whatever the beta script renders as a watermark/HUD: cleaner
       glass card, drop shadow, readable typography, viewport-clamped drag,
       does not cover the mobile composer, and a tap toggles a mini mode.
    ----------------------------------------------------------------------- */
    (function watermarkPolish() {
        const css = document.createElement('style');
        css.id = 'cc-watermark-polish';
        css.textContent = `
            /* Match common watermark roots used by the beta script */
            #cc-watermark, [id^="cc-watermark"], [class*="cc-watermark"],
            #corruptWatermark, .corrupt-watermark {
                font-family: ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif !important;
                font-size: 12px !important;
                line-height: 1.35 !important;
                color: #f5f7fb !important;
                background: linear-gradient(135deg, rgba(20,22,34,0.78), rgba(40,18,60,0.78)) !important;
                -webkit-backdrop-filter: blur(14px) saturate(160%);
                backdrop-filter: blur(14px) saturate(160%);
                border: 1px solid rgba(255,255,255,0.10) !important;
                border-radius: 14px !important;
                padding: 10px 14px !important;
                box-shadow: 0 10px 30px rgba(0,0,0,0.45), 0 0 0 1px rgba(120,90,255,0.15) inset !important;
                min-width: 160px;
            ...
            /* Headline accent for first child line if it looks like a title */
            #cc-watermark .cc-title, [class*="cc-watermark"] .cc-title {
            ...
            .cc-wm-mini > *:not(.cc-wm-dot) { display: none !important; }
            .cc-wm-dot {
                width: 12px; height: 12px; border-radius: 50%;
                background: radial-gradient(circle at 30% 30%, #a78bfa, #6d28d9);
                box-shadow: 0 0 12px #a78bfa, 0 0 2px #fff inset;
                display: inline-block;
            }
            /* Don't cover mobile composer */
        ...
        const enhance = (el) => {
            if (!el || el.dataset.ccEnhanced === '1') return;
            el.dataset.ccEnhanced = '1';

            // Inject a tiny status dot for mini mode
            if (!el.querySelector('.cc-wm-dot')) {
                const dot = document.createElement('span');
                dot.className = 'cc-wm-dot';
                dot.title = 'CorruptControl';
                el.prepend(dot);
            }

            // Try to mark a title line
            const firstText = [...el.childNodes].find(n => n.nodeType === 1 && n.textContent.trim());
            if (firstText && !firstText.classList.contains('cc-title')) {
                firstText.classList.add('cc-title');
            }

            // Tap/click toggles mini mode (separate from drag)
            ...
            el.addEventListener('pointerup', (e) => {
                if (!moved) el.classList.toggle('cc-wm-mini');
            });

            // Clamp on resize so it never escapes the viewport
            const clamp = () => {
                const r = el.getBoundingClientRect();
                const maxX = window.innerWidth - r.width - 4;
                const maxY = window.innerHeight - r.height - 4;
                const x = Math.max(4, Math.min(parseFloat(el.style.left) || r.left, maxX));
                const y = Math.max(4, Math.min(parseFloat(el.style.top) || r.top, maxY));
        ...
        const scan = () => {
            document.querySelectorAll(
                '#cc-watermark, [id^="cc-watermark"], [class*="cc-watermark"], #corruptWatermark, .corrupt-watermark'
            ).forEach(enhance);
        };
    ...
    /* ---------- Diagnostics ------------------------------------------------ */
    try {
        console.log('%c[CC-ELITE 2.1.0]%c enhancement layer active',
            'color:#a78bfa;font-weight:bold', 'color:#aaa');
    } catch (_) {}
})();

/* ============================================================================
   ORIGINAL CorruptControl 2.0.6 LOADER â€” VERBATIM, NOTHING REMOVED
   Source: https://raw.githubusercontent.com/glokk2pazzed-cpu/Test/refs/heads/main/corruptloader.js
   The script below still loads corrupt.beta.js (which contains the key
   system and the encrypted main script), exactly as before.
   ============================================================================ */
...
COMPLETE REWRITE - The Definitive Edition
  [+] TOTAL COMMAND SYSTEM OVERHAUL â€” Discord React-native input detection
  [+] 350+ COMMANDS â€” fully sorted by category with intelligent aliases
  [+] VIP WATERMARK SYSTEM â€” 12 effect modes, neon glow, gradient text
  [+] CYBERPUNK BACKGROUND ENGINE â€” matrix, particles, rain, snow, glitch, plasma
  [+] EXTERNAL SCRIPT LOADER â€” v2.0.4 style preserved (corrupt.beta.js + toggle.js)
  [+] COMMAND PALETTE â€” fuzzy search, category tabs, usage preview
  [+] CUSTOM EMOJI SYSTEM â€” watermark badges with animated indicators
  [+] REAL-TIME CLOCK â€” watermark shows live time/date
  [+] NETWORK MONITOR â€” live ping, packet loss, connection quality
  [+] ADVANCED STATISTICS â€” messages sent, commands used, uptime tracker
  [+] ENHANCED iOS/MOBILE SUPPORT â€” passive events, touch optimization
  [+] MEMORY MANAGEMENT â€” automatic cleanup, observer disposal, leak prevention
  [+] ERROR BOUNDARIES â€” try/catch on every command, graceful degradation
  [+] RATE LIMIT INTELLIGENCE â€” jitter backoff, queue system, smart retry
  [+] MODULAR ARCHITECTURE â€” clean separation, no globals, event-driven
  [+] ANTI-DETECTION â€” WebSocket frame filtering, request sanitization
  [+] CUSTOM CSS ENGINE â€” dynamic stylesheet injection, theme variables
  [+] DRAGGABLE WATERMARK â€” snap-to-corner, position memory
  [+] MINI MODE â€” collapsible watermark for minimal distraction
  [+] NOTIFICATION SYSTEM â€” toast messages for all actions
  [+] SOUND EFFECTS â€” optional audio feedback on commands
  [+] EXPORT/IMPORT â€” full settings backup and restore
  [+] AUTO-UPDATE CHECK â€” version checking with changelog display
=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=
*/
...
(function () {
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
       SECTION 0: EXTERNAL SCRIPT LOADER (v2.0.6 â€” Hardened)
       â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
    ...
    /* â€”â€”â€” FX Engine Configuration â€”â€”â€” */
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
       SECTION 7: COMMAND PROCESSOR (REWRITE â€” CORE FIX)
       â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
    ...
    async function handleKeydown(e) {
        ...
        if (commands[cmd]) {
            ...
            if (aliased && commands[aliased]) {
                ...
                await sendMessage('Unknown command: `' + prefix + cmd + '` â€” Use `' + prefix + 'help` for list.');
            }
        }
    ...
    /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
       SECTION 8: COMMAND REGISTRY â€” REGULAR COMMANDS (180+)
       â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
    ...
    const REGULAR_COMMANDS = {
        ...
        help(a) {
            if (a && a[0]) {
                ...
                if (all[a[0].toLowerCase()]) return '**\u2699\uFE0F ' + SCRIPT_NAME + '** â€” `.' + a[0] + ' [args]`\nCategory: *Use .cmds to browse all*';
...
**${Object.keys(REGULAR_COMMANDS).length}** regular + **${Object.keys(COHOST_COMMANDS).length - Object.keys(REGULAR_COMMANDS).length}** cohost commands`;
        ...
        cmds() {
            return `**\uD83D\uDCCB ${SCRIPT_NAME} v${VERSION} â€” COMMAND LIST**
...
\uD83D\uDD10 Cohost commands: Use **\`!help\`** (${Object.keys(COHOST_COMMANDS).length - Object.keys(REGULAR_COMMANDS).length}+ cmds)`;
        ...
        horoscope(a) {
            ...
            const readings = ['Today brings exciting opportunities. Stay open to change.', 'Focus on your goals â€” progress is imminent.', 'A meaningful conversation will brighten your day.', 'Creativity flows through you today.', 'Trust your intuition in matters of the heart.', 'Financial prospects look promising.'];
            return '\uD83C\uDF19 **' + s + '**\n' + readings[Math.floor(Math.random() * readings.length)];
        },

        roast(a) {
            const r = ["You're like a cloud. When you disappear, it\u2019s a beautiful day.", "I\u2019d agree with you but then we\u2019d both be wrong.", "You\u2019re not stupid; you just have bad luck thinking.", "Somewhere out there, a tree is producing oxygen for you. You owe it an apology.", "You bring everyone so much joy â€” when you leave the room.", "I'm jealous of people who don't know you.", "If I had a dollar for every brain you don't have, I'd have one dollar.", "You're proof that evolution can go in reverse.", "I'd explain it to you but I left my crayons at home.", "You're like a software update â€” nobody wants you."];
        ...
        leaderboard() {
            return '\uD83C\uDFC6 **Leaderboard**\n\uD83E\uDD47 ' + (state.username || 'You') + ' â€” Level ' + state.economy.level + ' (' + state.economy.balance.toLocaleString() + ' coins)';
        },
...
\uD83D\uDCC5 **Created:** ${created}`;
        ...
        async avatar(a) {
            ...
            return `\uD83D\uDDBC\uFE0F **Avatar** â€” ${u.global_name || u.username}
https://cdn.discordapp.com/avatars/${u.id}/${u.avatar}.${u.avatar.startsWith('a_') ? 'gif' : 'png'}?size=4096`;
        ...
        async banner(a) {
            ...
            return `\uD83D\uDDBC\uFE0F **Banner** â€” ${u.global_name || u.username}
...
${sorted.slice(0, 40).map(x => '- ' + x.name).join('\n')}`;
        ...
        async channels() {
            ...
            return `\uD83D\uDCC1 **Channels** â€” ${text.length} text, ${voice.length} voice
...
${m.slice(0, 30).map(x => '- ' + (x.user.global_name || x.user.username)).join('\n')}`;
        ...
        weather() {
            ...
            return w.icon + ' **Weather:** ' + w.desc + ' â€” **' + w.temp + '\u00B0F** (' + Math.round((w.temp - 32) * 5 / 9) + '\u00B0C)';
        },
...
\u2705 Advanced settings with export/import`;
        ...
        version() { return '\uD83D\uDE80 **' + SCRIPT_NAME + ' v' + VERSION + '** â€” ' + (state.isOwner ? '\uD83D\uDC51 Owner' : state.isCohost ? '\uD83D\uDC65 Cohost' : '\uD83D\uDC64 User') + ' Mode'; },
...
**\uD83D\uDCA5 SPAM** (6) â€” spam, burst, megaburst, ultraburst, godburst, nukeburst
**\uD83D\uDD28 MOD** (10) â€” ban, unban, kick, mute, unmute, purge, purgeuser, massban, masskick, massmute
**\uD83D\uDCC1 CHANNEL** (6) â€” lock, unlock, slowmode, clearslowmode, createchannel, deletechannel
**\uD83C\uDF97 ROLE** (7) â€” createrole, deleterole, rolecolor, massrole, addrole, removerole, clone roles
**\uD83D\uDC64 USER** (8) â€” nick, deafen, undeafen, servermute, serverunmute, disconnect, move, avatarall
**\uD83D\uDD0A VOICE** (4) â€” voicekick, voicemoveall, voicemuteall, voicedeafall
**\uD83D\uDC8C DM** (4) â€” dm, dmspam, massdm, dmraid
**\uD83D\uDD17 WEBHOOK** (7) â€” webhook, createhook, sendhook, webhookspam, webhooknuke, webhookdelete, webhookinfo
**\uD83D\uDEA8 NUKE** (4) â€” nukechannels, nukeroles, massnick, massdelete
**\u270F\uFE0F MESSAGE** (6) â€” editmsg, pin, unpin, crosspost, reactspam, clearreacts
**\uD83D\uDC51 ADMIN** (10) â€” addcohost, removecohost, listcohosts, clearcohosts, setowner, typing, eval, autoreact, autodel, hidden
**\uD83D\uDD27 UTIL** (8) â€” tokeninfo, autoping, autonick, autoreply, cloneserver, auditlog, ghostping, clearlog
**\uD83D\uDDBC\uFE0F TEXT** (5) â€” wall, lines, flood, emojispam, mentionspam
...
Type **\`!help\`** for the full command list.`;
        ...
        async nukeburst(a) {
            ...
            return '\u2620\uFE0F **NUCLEAR: ' + count + 'x** â€” Hold on tight...';
        },
...
${state.ghostPings.slice(-10).map((gp, i) => (i + 1) + '. <@' + gp.author + '> â€” ' + $.timeAgo(gp.time)).join('\n')}`;
        },
    ...
    function buildEffectsSection() {
        ...
        return '<div id="cc-effects" class="cc-section">' +
            '<div class="cc-info-box" style="margin-bottom:14px;">\uD83C\uDFA8 <strong>Background FX Engine</strong> â€” Select an effect and customize its appearance. Effects run behind the Discord UI.</div>' +
    ...
    function buildBadgesSection() {
        ...
        let html = '<div id="cc-badges" class="cc-section"><div class="cc-info-box" style="margin-bottom:14px;">\uD83C\uDFC5 Toggle client-side badges. These are <strong>visual only</strong> â€” only you see them.</div>';
    ...
    function buildDataSection() {
        return '<div id="cc-data" class="cc-section">' +
            '<div class="cc-info-box" style="margin-bottom:14px;">\uD83D\uDCBE <strong>Backup & Restore</strong> â€” Export or import all your settings.</div>' +
    ...
    async function init() {
        ...
        // Inject systems â€” prioritize critical first
        injectFonts();
        buildWatermark();
        initCommandSystem();
        initScrollStability();
        ...
        console.log('[CC-Elite] â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
  ...
  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   EMERGENCY UNHIDE â€” Triple Tap Anywhere (iOS/Stay)
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
(function initEmergencyUnhide() {
    ...
    document.addEventListener('touchend', (e) => {
        ...
        if (tapCount >= 3) {
            ...
            Settings.save('watermark', WatermarkCfg);
            buildWatermark();
            showToast('ðŸ”“ Watermark restored!', 'success', 3000);
        }
